        ...
        <div className={style.contenedorMayor}>
            <div className={style.contenedor} >
                ...
                <form>
                    ...
                    {existe !== true ? (<button className={style.formbutton} onClick={(event) => userAdmin(emailInput, event)}>Siguiente</button>) : (null)}
                    {existe === true ? (
                    ...
                    {existe === true ? (<NavLink to="/dashboard"> <button className={style.formbutton} onClick={(event) => loginAdmin(emailInput, password, event)}>Inciar sesión</button></NavLink>
                    ) : (null)}
                    {handleRedirect ()}
                    {adminPass === false ? (
                    <div className={style.divmensajePass}>
                    <span className={style.span}> &#9888; Correo electronico y contraseña incorrectas </span>
                    </div>): (null)}
                    {existe === false ? (<div className={style.divmensaje}>
                        <span className={style.span}> &#9888; Este usuario no es administrador</span>
